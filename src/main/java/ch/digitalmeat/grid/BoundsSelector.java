...
public class BoundsSelector<T extends ChunkTileBase<T, C>, C extends ChunkBase<T, C>> implements Selector<T> {
	...
	public List<T> select(Bounds bounds) {
		return select(bounds.x, bounds.y, bounds.x2(), bounds.y2());
	}
